...
def run_backtest(experiment: str, implementation: str) -> str:
    ...
    update_backtest_status(experiment, implementation, is_complete=True)
    
    return f"Backtest completed for {experiment} - {implementation}"

def update_approval_status(experiment: str, implementation: str, approval_type: str, approve: bool = True) -> str:
    """
    ...
    # Update the SQLite database
    update_experiment_approval(experiment, implementation, approval_type, approve)
    
    action = "approved" if approve else "revoked"
    return f"{approval_type.capitalize()} {action} for {experiment} - {implementation}"
...
def check_approved(exp: str, impl: str, approval_type: str) -> bool:
    """
    Checks if an experiment-implementation has been approved.
...
def create_summary_dataframe() -> pd.DataFrame:
    ...
    for experiment in unique_experiments:
        ...
        for impl in exp_df['implementation'].unique():
            ...
            config_approved = check_approved(experiment, impl, 'config')
            backtest_complete = impl_df['is_backtest_complete'].iloc[0]
            comparison_approved = check_approved(experiment, impl, 'comparison')
            final_approved = check_approved(experiment, impl, 'final_summary')
